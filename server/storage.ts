...
import { 
  ...
  type User, type Product, type Review, type Order, type ProductImage,
...
export class DatabaseStorage implements IStorage {
    ...
    return value
      .map((item): ProductImage | null => {
        ...
        if (item && typeof item === "object") {
          const url = typeof (item as any).url === "string" ? String((item as any).url).trim() : "";
          if (!url) return null;
          const rawThumb = typeof (item as any).thumb === "string" ? String((item as any).thumb).trim() : "";
          return rawThumb ? { url, thumb: rawThumb } : { url };
        }
        return null;
      })
  ...
  async updateProduct(id: number, updates: UpdateProductRequest): Promise<Product | undefined> {
    ...
    if ("images" in updates) {
      payload.images = this.normalizeImages(updates.images);
    }
    if ("specs" in updates) {
